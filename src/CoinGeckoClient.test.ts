...
describe("CoinGeckoClient test", () => {
  ...
  describe("Coins", () => {
    ...
    it("/coins/{id}/history should successful", async () => {
      const coin = await client.coinIdHistory({
        id: "bitcoin",
        date: "01-04-2021",
      });
    ...
    it("/coins/{id}/ohlc should successful", async () => {
      const ohlc = await client.coinIdOHLC({
        id: "litecoin",
        vs_currency: "aud",
        days: 30,
      });
  ...
  describe("Simple", () => {
    ...
    it("/simple/prices should successful", async () => {
      const list = await client.simplePrice({
        vs_currencies: "eth",
        ids: "bitcoin",
      });
      expect(list).toEqual({
  ...
  describe("Exchange", () => {
    ...
    it("/exchange/id/volume_chart should successful", async () => {
      const aave = await client.exchangeIdVolumeChart({
        id: "whitebit",
        days: 1,
      });
  ...
  describe("Derivatives", () => {
    ...
    it("/derivatives/exchanges should successful", async () => {
      const list = await client.derivativesExchanges({
        order: "name_asc",
        per_page: 10,
      });
      expect(list.length).toBeGreaterThan(0);
    ...
    it("/derivatives/exchanges/id should successful", async () => {
      const item = await client.derivativesExchangesId({
        id: "binance_futures",
      });
      expect(item).toEqual({
